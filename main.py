...
from telethon.tl.functions.account import (
    UpdateProfileRequest, SetPrivacyRequest, UpdateNotifySettingsRequest
)
...
def speed_for(uid):
    ...
    if not s:
        return (MIN_INTERVAL, MAX_INTERVAL, CYCLE_WAIT)
    mn = s.get('min', MIN_INTERVAL)
    mx = s.get('max', MAX_INTERVAL)
    cyc = s.get('cycle', CYCLE_WAIT)
    try: mn = float(mn)
    except: mn = float(MIN_INTERVAL)
    try: mx = float(mx)
    except: mx = float(MAX_INTERVAL)
    try: cyc = float(cyc)
    except: cyc = float(CYCLE_WAIT)
    return (mn, mx, cyc)


def set_speed(uid, min_i=None, max_i=None, cycle=None):
    d = load_user_speeds()
    s = d.setdefault(str(uid), {})
    if min_i is not None: s['min'] = float(min_i)
    if max_i is not None: s['max'] = float(max_i)
    if cycle is not None: s['cycle'] = float(cycle)
    save_user_speeds(d)
...
def save_backups(d):
    try: json.dump(d, open(BACKUP_FILE, 'w'), indent=2)
    except: pass


def get_user_backup(uid):
    b = load_backups().get(str(uid))
    if not b:
        b = {
            'enabled': False,
            'backup_ids': [],
...
def in_backup_window(cfg):
    start = parse_hhmm(cfg.get('start_time', '03:00'))
    end = parse_hhmm(cfg.get('end_time', '09:00'))
...
def parse_admin_cmd(t):
    ...
    uid = int(pr[0])
    rest = pr[1].strip() if len(pr) > 1 else 'perm'
    op = '+'
    ...
    for acc_id, api_id, api_hash, session in acc_configs:
        if api_id and api_hash and session:
            try:
                ...
                await c.start()
                me = await c.get_me()
                n = me.first_name or f"User{me.id}"
                await c.disconnect()
...
def get_all_accounts(user_id=None):
    ...
    for s in load_auth_sessions():
        auth.append({'id': s['id'], 'name': s.get('name', f"User_{s.get('user_id','?')}"),
...
def add_dynamic_account(name, ss, owner_id, api_id=0, api_hash=""):
    ...
    save_dynamic_accounts(accs)
    display_names[nid] = name
    return True, nid
...
def account_limit_reached(u):
    ...
    if owner_acc_count(u) >= cap:
        return True, f"❌ Account limit reached ({owner_acc_count(u)}/{cap})!"
    return False, ""
...
def save_data():
    try:
        json.dump({'message': MESSAGE, 'min_interval': MIN_INTERVAL, 'max_interval': MAX_INTERVAL,
                   'cycle_wait': CYCLE_WAIT, 'show_start_to_others': SHOW_START_TO_OTHERS},
...
async def apply_profile(acc, name, photo, bio, channels, bot=None, apply_privacy=True):
    ...
    if apply_privacy:
        try:
            ...
            r.append(f"❌ Privacy: {str(e)[:40]}")
        try:
            muted = await mute_all_notifications(client)
...
async def apply_profile_specific(acc_id, owner_id, bot=None):
    acc = None
    for a in get_all_accounts(owner_id):
        if a['id'] == acc_id:
            acc = a
            break
    if not acc:
    ...
    bio = cfg.get('bio', '')
    ch = cfg.get('channels', [])
    if not nm and not ph and not bio and not ch:
        return ["❌ No default profile config set!"]
    name = nm[0] if nm else ''
    photo = ph[0] if ph else None
    try:
        rr = await apply_profile(acc, name, photo, bio, ch, bot=bot)
        return rr
    except Exception as e:
        return [f"❌ {str(e)[:120]}"]
...
# ============ SEND + MESSAGING ============
async def send_to_group(client, group, owner, account_id):
    try:
        msg = get_random_message_for(owner)
        rt = await get_reply_target(client, group)
        if rt:
            await client.send_message(group, msg, reply_to=rt.id)
        else:
            await client.send_message(group, msg)
        account_stats[account_id]['sent'] += 1
        return True
    except FloodWaitError as e:
        asyncio.create_task(asyncio.sleep(min(e.seconds, 120)))
        return False
...
async def run_account_messaging(acc, owner):
    ...
    try:
        ...
        while not stop_flags.get(aid, False):
            ...
            active_groups = [g for g in groups if g.id not in failed]
            ...
            if is_super_fast:
                ...
                async def worker(g):
                    if stop_flags.get(aid): return
                    async with sem:
                        if stop_flags.get(aid): return
                        ok = await send_to_group(client, g, owner, aid)
                        if not ok:
                            await asyncio.sleep(random.uniform(float(mn), float(mx)))

                tasks = [asyncio.create_task(worker(g)) for g in active_groups]
...
async def backup_switch_loop():
    while True:
        try:
            await asyncio.sleep(60)
            now_ist = ist_now()
            today = now_ist.strftime('%Y-%m-%d')
            all_backups = load_backups()
            for uid_str, cfg in list(all_backups.items()):
                try:
                    uid = int(uid_str)
                except Exception:
                    continue
                ...
                del_at = cfg.get('delete_after')
                if del_at:
                    try:
                        if now_ist >= datetime.fromisoformat(del_at):
                            logger.info(f"🗑️ Backup auto-delete for {uid}")
                            cfg['enabled'] = False
                            cfg['backup_ids'] = []
                            cfg['delete_after'] = None
                            set_user_backup(uid, cfg)
                            await notify_user(uid, "🗑️ *Backup config auto-deleted* (scheduled)")
                            continue
                    except Exception:
                        pass

                in_window = in_backup_window(cfg)
                last_date = cfg.get('last_run_date')

                if in_window and last_date != f"in_{today}":
                    logger.info(f"🌙 Backup window START {uid}")
                    for a in get_all_accounts(uid):
                        if a['id'] not in cfg['backup_ids']:
                            stop_account(a['id'])
                    for bid in cfg['backup_ids']:
                        acc = None
                        for a in get_all_accounts(uid):
                            if a['id'] == bid:
                                acc = a
                                break
                        if acc and not account_stats.get(bid, {}).get('running', False):
                            stop_flags[bid] = False
                            running_tasks[bid] = asyncio.create_task(run_account_messaging(acc, uid))
                    cfg['last_run_date'] = f"in_{today}"
                    set_user_backup(uid, cfg)
                    await notify_user(uid, f"🌙 *Backup mode ON* ({cfg['start_time']}–{cfg['end_time']} IST)")

                elif (not in_window) and last_date == f"in_{today}":
                    logger.info(f"☀️ Backup window END {uid}")
                    for bid in cfg['backup_ids']:
                        stop_account(bid)
                    for a in get_all_accounts(uid):
                        if a['id'] not in cfg['backup_ids'] and not account_stats.get(a['id'], {}).get('running', False):
                            stop_flags[a['id']] = False
                            running_tasks[a['id']] = asyncio.create_task(run_account_messaging(a, uid))
                    cfg['last_run_date'] = f"out_{today}"
                    set_user_backup(uid, cfg)
                    await notify_user(uid, f"☀️ *Backup mode OFF* — main accounts active")
        except Exception as e:
            logger.error(f"backup loop err: {e}")
...
async def test_session_only(ss):
    ...
    try:
        if not API_ID_1 or not API_HASH_1:
            return False, "API missing", None, None
...
def main_menu_text(u):
    ...
    fast = "🚀 SUPER FAST" if mn <= 0.2 else "🐢 Normal"
...
async def start_command(u, c):
    ...
    if SHOW_START_TO_OTHERS:
        await u.message.reply_text("🤖 Private bot. Please contact the owner.")
...
async def button_click(u, c):
    ...
    if not (is_owner(uid) or is_valid_admin(uid)):
        if SHOW_START_TO_OTHERS:
            await q.edit_message_text("⛔ Access denied / expired.")
        else:
    ...
    if d == 'start_all':
        ...
        for a in get_all_accounts(uid):
            if account_stats.get(a['id'], {}).get('running', False):
                stop_account(a['id'])
                p.append(f"⏹️ Stopping: {get_display_name(a)}")
            else:
                p.append(f"⏸️ Stopped: {get_display_name(a)}")
        ...
        mn, mx, cyc = speed_for(uid)
        if mn <= 0.2:
            set_speed(uid, min_i=MIN_INTERVAL, max_i=MAX_INTERVAL, cycle=CYCLE_WAIT)
            new_label = "OFF"
        else:
            set_speed(uid, min_i=0.05, max_i=0.15, cycle=5)
            new_label = "ON"
        await q.edit_message_text(
            f"⚡ *Super Fast Mode: {new_label}*\n\n"
            f"{'🚀 1 second-এ ~10-20 message যাবে (parallel workers)' if new_label == 'ON' else '🐢 Normal speed'}\n\n"
            f"⚠️ Fast mode-এ Telegram দ্রুত ban করতে পারে!",
        ...
        mn, mx, cyc = speed_for(uid)
        kb = [
            [InlineKeyboardButton(f"⏱️ Min {mn}s", callback_data='set_min'),
             InlineKeyboardButton(f"⏱️ Max {mx}s", callback_data='set_max')],
            [InlineKeyboardButton(f"🔄 Cycle {cyc}s", callback_data='set_cycle')],
            [InlineKeyboardButton("🚀 Set Super Fast (0.05-0.15)", callback_data='set_super_fast')],
            [InlineKeyboardButton("🔙 Back", callback_data='settings')]
        ]
        await q.edit_message_text(
            "⏱️ *Speed Settings*\n\nFloat value সাপোর্ট করে (যেমন 0.05, 0.1)",
            parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))

    elif d == 'set_super_fast':
        set_speed(uid, min_i=0.05, max_i=0.15, cycle=5)
        await q.edit_message_text(
            "🚀 *SUPER FAST Enabled*\n\n"
            "Speed: 0.05-0.15s\nCycle: 5s\nConcurrency: 10 parallel workers\n\n"
            "মানে: প্রতি second-এ ~10 message যাবে!",
            parse_mode='Markdown', reply_markup=BACK_KB)

    elif d == 'add_message':
        c.user_data['awaiting'] = 'add_message'
        await q.edit_message_text("✏️ Send the new message text:", reply_markup=BACK_KB)

    elif d == 'delete_message_menu':
        m = load_messages_for(uid)
        if not m:
            await q.edit_message_text("❌ No messages yet", reply_markup=BACK_KB)
            return
        ...
        m = load_messages_for(uid)
        i = int(d.replace('del_msg_', ''))
        if 0 <= i < len(m):
            m.pop(i); save_messages_for(uid, m)
        await q.edit_message_text("🗑️ Deleted", reply_markup=BACK_KB)

    elif d == 'reset_messages':
        save_messages_for(uid, [MESSAGE])
        await q.edit_message_text("🔄 Reset done", reply_markup=BACK_KB)

    elif d == 'set_min':
        mn, mx, cyc = speed_for(uid)
        c.user_data['awaiting'] = 'set_min'
        await q.edit_message_text(f"⏱️ Enter Min seconds (0.01 - {mx}):", reply_markup=BACK_KB)

    elif d == 'set_max':
        mn, mx, cyc = speed_for(uid)
        c.user_data['awaiting'] = 'set_max'
        await q.edit_message_text(f"⏱️ Enter Max seconds (> {mn}):", reply_markup=BACK_KB)

    elif d == 'set_cycle':
        c.user_data['awaiting'] = 'set_cycle'
        await q.edit_message_text("🔄 Enter cycle seconds (5+):", reply_markup=BACK_KB)
        ...
        ph = cfg.get('photos', [])
        if not accs:
            await q.edit_message_text("❌ No accounts", reply_markup=BACK_KB)
            return
        kb = [
            [InlineKeyboardButton(f"⚙️ Profile Config (Names:{len(nm)}|Logos:{len(ph)})", callback_data='profdefault')],
            [InlineKeyboardButton("⚡ Apply to ALL accounts", callback_data='profapply_all')],
            [InlineKeyboardButton("🎯 Apply to ONE account", callback_data='profapply_one_menu')],
            [InlineKeyboardButton("🔙 Back", callback_data='back_main')]
        ]
        await q.edit_message_text(
            f"🎨 *Profile Setup*\nAccounts: {len(accs)}\n\n"
            f"⚡ = সব account-এ apply\n🎯 = একটা specific account-এ apply",
            parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))

    elif d == 'profapply_one_menu':
        accs = get_all_accounts(uid)
        if not accs:
            await q.edit_message_text("❌ No accounts", reply_markup=BACK_KB)
            return
        ...
        for i, a in enumerate(accs, 1):
            ...
            kb.append([InlineKeyboardButton(
                f"{ti} #{i} · {get_display_name(a)[:22]}",
                callback_data=f'profapply_one_{a["id"]}'
            )])
        kb.append([InlineKeyboardButton("🔙 Back", callback_data='profile_setup')])
        await q.edit_message_text("🎯 *Select account:*", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))

    elif d.startswith('profapply_one_'):
        acc_id = d.replace('profapply_one_', '')
        await q.edit_message_text("⏳ Applying to 1 account...", reply_markup=BACK_KB)
        try:
            rr = await apply_profile_specific(acc_id, uid, bot=c.bot)
            txt = "✅ *Done!*\n\n" + "\n".join(rr)
            await q.edit_message_text(txt, parse_mode='Markdown', reply_markup=BACK_KB)
        except Exception as e:
            await q.edit_message_text(f"❌ {str(e)[:150]}", reply_markup=BACK_KB)
        ...
        txt = (f"⚙️ *Profile Config*\n"
               f"📝 Names ({len(nm)})\n"
               f"🖼️ Logos: {len(ph)} | 📄 Bio: `{cfg.get('bio','-')}`\n"
               f"📢 Channels ({len(ch)})\n" + "".join(f" {x}\n" for x in ch[:10]))
        kb = [
            [InlineKeyboardButton("➕ Add Name", callback_data='def_add_name'),
             InlineKeyboardButton("➖ Del Name", callback_data='def_del_name')],
            [InlineKeyboardButton("🖼️ Add Logo", callback_data='def_add_photo'),
             InlineKeyboardButton("🗑️ Del Logo", callback_data='def_del_photo')],
        ...
        c.user_data['awaiting'] = 'def_add_name'
        await q.edit_message_text("📝 Send names, one per line:", reply_markup=BACK_KB)

    elif d == 'def_del_name':
        nm = get_default_profile().get('names', [])
        if not nm:
            await q.edit_message_text("❌ No names", reply_markup=BACK_KB)
            return
        ...
        cfg = get_default_profile()
        nm = cfg.get('names', [])
        i = int(d.replace('def_delname_', ''))
        if 0 <= i < len(nm):
            nm.pop(i); cfg['names'] = nm; save_default_profile(cfg)
        await q.edit_message_text("🗑️ Deleted", reply_markup=BACK_KB)

    elif d == 'def_add_photo':
        c.user_data['awaiting'] = 'def_add_photo'
        await q.edit_message_text("🖼️ Send a photo:", reply_markup=BACK_KB)

    elif d == 'def_del_photo':
        ph = get_default_profile().get('photos', [])
        if not ph:
            await q.edit_message_text("❌ No logos", reply_markup=BACK_KB)
            return
        ...
        cfg = get_default_profile()
        ph = cfg.get('photos', [])
        i = int(d.replace('def_delphoto_', ''))
        if 0 <= i < len(ph):
            ph.pop(i); cfg['photos'] = ph; save_default_profile(cfg)
        await q.edit_message_text("🗑️ Deleted", reply_markup=BACK_KB)

    elif d == 'def_bio':
        c.user_data['awaiting'] = 'def_bio'
        await q.edit_message_text("📄 Send bio text:", reply_markup=BACK_KB)

    elif d == 'def_chan':
        c.user_data['awaiting'] = 'def_chan'
        await q.edit_message_text("📢 Send channel links (line wise):", reply_markup=BACK_KB)
        ...
        if not accs or (not nm and not ph and not bio and not ch):
            await q.edit_message_text("❌ Add config first!", reply_markup=BACK_KB)
            return
        total = len(accs)
        prog = {'done': 0, 'lines': {}}
    ...
    # ============ BACKUP MENU ============
        ...
        cfg = get_user_backup(uid)
        status = "🟢 ON" if cfg.get('enabled') else "🔴 OFF"
        n_ids = len(cfg.get('backup_ids', []))
        del_at = cfg.get('delete_after') or "—"
        kb = [
            [InlineKeyboardButton(f"🔘 Status: {status}", callback_data='backup_toggle')],
            [InlineKeyboardButton(f"📦 Backup IDs ({n_ids})", callback_data='backup_ids_menu')],
            [InlineKeyboardButton(f"🕐 Start: {cfg.get('start_time','—')} IST", callback_data='backup_set_start')],
            [InlineKeyboardButton(f"🕐 End: {cfg.get('end_time','—')} IST", callback_data='backup_set_end')],
            [InlineKeyboardButton(f"🗑️ Delete at: {del_at[:19]}", callback_data='backup_set_delete')],
            [InlineKeyboardButton("🧪 Test Now", callback_data='backup_test')],
            [InlineKeyboardButton("❌ Clear Backup", callback_data='backup_clear')],
            [InlineKeyboardButton("🔙 Back", callback_data='back_main')],
        ]
        await q.edit_message_text(
            f"🌙 *Backup Manager*\n\n"
            f"Status: {status}\nBackup IDs: {n_ids}\n"
            f"Window: {cfg.get('start_time')} → {cfg.get('end_time')} (IST)\n"
            f"Auto-delete at: {del_at}\n\n"
            f"⏰ Start-এ main ID stop, backup ID start\n"
            f"⏰ End-এ উল্টোটা",
            parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))

    elif d == 'backup_toggle':
        cfg = get_user_backup(uid)
        ...
        await q.edit_message_text(f"✅ Backup: {status}", reply_markup=BACK_KB)

    elif d == 'backup_ids_menu':
        cfg = get_user_backup(uid)
        all_accs = get_all_accounts(uid)
        selected = set(cfg.get('backup_ids', []))
        kb = []
        for a in all_accs:
            aid = a['id']
            ...
            ti = {'env': '💚', 'dynamic': '💙', 'phone_auth': '📱'}.get(a.get('type', ''), '❓')
            kb.append([InlineKeyboardButton(
                f"{mark} {ti} {get_display_name(a)[:22]}",
                callback_data=f'backup_tog_{aid}'
            )])
        kb.append([InlineKeyboardButton("🗑️ Clear All", callback_data='backup_ids_clear')])
        kb.append([InlineKeyboardButton("🔙 Back", callback_data='backup_menu')])
        await q.edit_message_text(
            f"📦 *Select Backup IDs* ({len(selected)} selected)\n\n"
            f"যেগুলো টিক দিবে সেগুলোই backup হিসেবে কাজ করবে",
            parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))

    elif d.startswith('backup_tog_'):
        aid = d.replace('backup_tog_', '')
        cfg = get_user_backup(uid)
        ids = cfg.get('backup_ids', [])
        if aid in ids:
            ids.remove(aid)
        else:
            ids.append(aid)
        ...
        for a in all_accs:
            ...
            kb.append([InlineKeyboardButton(
                f"{mark} {ti} {get_display_name(a)[:22]}",
                callback_data=f'backup_tog_{a["id"]}'
            )])
        kb.append([InlineKeyboardButton("🗑️ Clear All", callback_data='backup_ids_clear')])
        kb.append([InlineKeyboardButton("🔙 Back", callback_data='backup_menu')])
        await q.edit_message_text(
    ...
    elif d == 'backup_ids_clear':
        cfg = get_user_backup(uid)
        cfg['backup_ids'] = []
        set_user_backup(uid, cfg)
        await q.edit_message_text("🗑️ Backup IDs cleared", reply_markup=BACK_KB)

    elif d == 'backup_set_start':
        c.user_data['awaiting'] = 'backup_start'
        await q.edit_message_text(
            "🕐 *Enter backup START time*\n\nFormat: `HH:MM` (24-hour, IST)\nExample: `03:00`",
            parse_mode='Markdown', reply_markup=BACK_KB)

    elif d == 'backup_set_end':
        c.user_data['awaiting'] = 'backup_end'
        await q.edit_message_text(
            "🕐 *Enter backup END time*\n\nFormat: `HH:MM` (24-hour, IST)\nExample: `09:00`",
            parse_mode='Markdown', reply_markup=BACK_KB)

    elif d == 'backup_set_delete':
        c.user_data['awaiting'] = 'backup_delete'
        await q.edit_message_text(
            "🗑️ *Auto-delete backup config*\n\n"
            "Format: `2h` / `30m` / `1d` / `off`",
            parse_mode='Markdown', reply_markup=BACK_KB)

    elif d == 'backup_test':
        cfg = get_user_backup(uid)
        if not cfg.get('backup_ids'):
            await q.edit_message_text("❌ Backup IDs সিলেক্ট করুন আগে", reply_markup=BACK_KB)
            return
        await q.edit_message_text("🧪 Testing backup switch...", reply_markup=BACK_KB)
        for a in get_all_accounts(uid):
            if a['id'] not in cfg['backup_ids']:
                stop_account(a['id'])
        for bid in cfg['backup_ids']:
            ...
            if acc and not account_stats.get(bid, {}).get('running', False):
                ...
                running_tasks[bid] = asyncio.create_task(run_account_messaging(acc, uid))
        await q.edit_message_text(
            f"✅ *Test switch done*\nMain stopped, {len(cfg['backup_ids'])} backups started",
            parse_mode='Markdown', reply_markup=BACK_KB)

    elif d == 'backup_clear':
        set_user_backup(uid, {
            'enabled': False, 'backup_ids': [],
            'start_time': '03:00', 'end_time': '09:00',
            'delete_after': None, 'last_run_date': None,
        })
        await q.edit_message_text("🗑️ Backup config cleared", reply_markup=BACK_KB)
    ...
    # ============ ADMIN PANEL ============
        ...
        admins = load_admins()
        if not admins:
            await q.edit_message_text("❌ No admins yet", reply_markup=BACK_KB)
            return
        txt = "📋 *Admins*\n\n"
        kb = []
        for a in admins:
        ...
        a = get_admin(t)
        if not a:
            await q.edit_message_text("❌ Not an admin", reply_markup=BACK_KB)
            return
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("+30 days", callback_data=f'admop_{t}_+30'),
             InlineKeyboardButton("+100 days", callback_data=f'admop_{t}_+100')],
            [InlineKeyboardButton("-10 days", callback_data=f'admop_{t}_-10'),
             InlineKeyboardButton("-30 days", callback_data=f'admop_{t}_-30')],
            [InlineKeyboardButton("♾️ Permanent", callback_data=f'admop_{t}_=perm'),
        ...
        if not is_owner(uid): return
        try:
            body = d.replace('admop_', '')
            t_s, op = body.rsplit('_', 1)
            target = int(t_s)
        except Exception:
            await q.edit_message_text("❌ Parse error", reply_markup=BACK_KB)
            return
        now = datetime.now()
        if op == 'perm':
            await apply_admin_time(target, '=', None, q=q)
        elif op == '0':
            a = get_admin(target)
            if a:
                a['expires_at'] = now.isoformat()
        ...
        for a in get_all_accounts(uid):
            ...
            stop_account(a['id'])
            remove_account_by_id(a['id'])
            await disconnect_client(a['id'])
            n += 1
        ...
        for a in get_all_accounts(uid):
            if a['id'] == acc_id:
                ...
                break
        if not t:
            await q.edit_message_text("⛔ invalid", reply_markup=BACK_KB)
            return
        ...
        for dd in (account_stats, stop_flags, running_tasks, display_names):
            dd.pop(acc_id, None)
...
async def handle_photo(u, c):
    ...
    if c.user_data.get('awaiting') == 'def_add_photo':
        ...
        c.user_data['awaiting'] = None
        await u.message.reply_text(f"🖼️ Logo #{len(ph)} saved", reply_markup=BACK_KB)
        return
...
async def handle_text(u, c):
    ...
    if aw == 'add_admin':
        ...
        try:
            ...
            await u.message.reply_text("❌ Format: `USER_ID [+|-|=]TIME`", parse_mode='Markdown', reply_markup=BACK_KB)
            return
        if target == OWNER_ID:
            await u.message.reply_text("❌ Owner cannot be edited.", reply_markup=BACK_KB)
            return
    ...
    if aw == 'admin_limit':
        ...
        try:
            p = text.split()
            t = int(p[0]); cap = int(p[1])
            if cap < 0: raise ValueError()
        except Exception:
            await u.message.reply_text("❌ `USER_ID NUMBER`", reply_markup=BACK_KB)
            return
        admins = load_admins()
        found = False
        for a in admins:
            if a['user_id'] == t:
                a['max_accounts'] = (None if cap == 0 else cap)
                found = True
        if not found:
            await u.message.reply_text(f"❌ Admin not found: {t}", reply_markup=BACK_KB)
            return
        save_admins(admins)
        await u.message.reply_text(f"✅ {admin_label(t)} limit: {'unlimited' if cap == 0 else str(cap)}", reply_markup=BACK_KB)
        return
    ...
    if aw == 'def_add_name':
        ...
        cfg['names'] = nm
        save_default_profile(cfg)
        await u.message.reply_text(f"✅ Added {len(nn)} (total {len(nm)})", reply_markup=BACK_KB)
        return
    ...
    if aw == 'def_bio':
        ...
        cfg['bio'] = text
        save_default_profile(cfg)
        await u.message.reply_text("✅ Bio saved", reply_markup=BACK_KB)
        return
    ...
    if aw == 'def_chan':
        ...
        save_default_profile(cfg)
        await u.message.reply_text("✅ Links saved", reply_markup=BACK_KB)
        return
    ...
    if aw == 'add_message':
        ...
        m = load_messages_for(uid)
        m.append(text)
        save_messages_for(uid, m)
        await u.message.reply_text(f"✅ Added, total {len(m)}", reply_markup=BACK_KB)
        return

    if aw == 'backup_start':
        c.user_data['awaiting'] = None
        hm = parse_hhmm(text.strip())
        if not hm:
            await u.message.reply_text("❌ Format: `HH:MM` (যেমন 03:00)", parse_mode='Markdown', reply_markup=BACK_KB)
            return
        cfg = get_user_backup(uid)
        cfg['start_time'] = f"{hm[0]:02d}:{hm[1]:02d}"
        cfg['last_run_date'] = None
        set_user_backup(uid, cfg)
        await u.message.reply_text(f"✅ Start time: {cfg['start_time']} IST", reply_markup=BACK_KB)
        return
    ...
    if aw == 'backup_end':
        ...
        hm = parse_hhmm(text.strip())
        if not hm:
            await u.message.reply_text("❌ Format: `HH:MM`", parse_mode='Markdown', reply_markup=BACK_KB)
            return
        cfg = get_user_backup(uid)
        cfg['end_time'] = f"{hm[0]:02d}:{hm[1]:02d}"
        set_user_backup(uid, cfg)
        await u.message.reply_text(f"✅ End time: {cfg['end_time']} IST", reply_markup=BACK_KB)
        return
    ...
    if aw == 'backup_delete':
        ...
        if text.strip().lower() == 'off':
            cfg['delete_after'] = None
            set_user_backup(uid, cfg)
            await u.message.reply_text("✅ Auto-delete: OFF", reply_markup=BACK_KB)
            return
        try:
            delta = parse_duration_delta(text.strip())
            if not delta: raise ValueError()
            del_at = (ist_now() + delta).isoformat()
            cfg['delete_after'] = del_at
            set_user_backup(uid, cfg)
            await u.message.reply_text(f"✅ Auto-delete: `{del_at[:19]}` (IST)", parse_mode='Markdown', reply_markup=BACK_KB)
        except Exception:
            await u.message.reply_text("❌ Format: `2h`, `30m`, `1d`, or `off`", parse_mode='Markdown', reply_markup=BACK_KB)
        return
    ...
    if aw == 'set_min':
        ...
        try:
            ...
            await u.message.reply_text(f"❌ 0.01 থেকে {mx} এর মধ্যে দিন", reply_markup=BACK_KB)
            return
        mn, mx, cyc = speed_for(uid)
        await u.message.reply_text(f"✅ Min {mn}s", reply_markup=BACK_KB)
        return
    ...
    if aw == 'set_max':
        ...
        try:
            ...
            await u.message.reply_text(f"❌ > {mn} হতে হবে", reply_markup=BACK_KB)
            return
        mn, mx, cyc = speed_for(uid)
        await u.message.reply_text(f"✅ Max {mx}s", reply_markup=BACK_KB)
        return
    ...
    if aw == 'set_cycle':
        ...
        try:
            ...
            await u.message.reply_text("❌ >= 5 হতে হবে", reply_markup=BACK_KB)
            return
        mn, mx, cyc = speed_for(uid)
        await u.message.reply_text(f"✅ Cycle {cyc}s", reply_markup=BACK_KB)
        return
    ...
    if aw == 'phone_number':
        ...
        try:
            ...
            if not re.match(r'^\+\d{7,15}$', ph):
                await u.message.reply_text("❌ Invalid! Example `+91XXXXXXXXXX`", reply_markup=BACK_KB)
                return
            reached, rm = account_limit_reached(uid)
            if reached:
                await u.message.reply_text(rm, reply_markup=BACK_KB)
                return
            if not API_ID_1 or not API_HASH_1:
                await u.message.reply_text("❌ API keys missing on server!", reply_markup=BACK_KB)
                return
            ...
            try:
                ...
                phone_login_states[lid] = {'phone': ph, 'api_id': API_ID_1, 'api_hash': API_HASH_1,
                                           'client': client, 'owner_id': uid,
                                           'phone_code_hash': sent.phone_code_hash, 'created': datetime.now()}
                c.user_data['login_id'] = lid
                c.user_data['awaiting'] = 'otp_code'
    ...
    if aw == 'otp_code':
        ...
        if not st:
            c.user_data['awaiting'] = None
            await u.message.reply_text("⏳ Flow reset. Phone Login again.", reply_markup=BACK_KB)
            return
        code = text.strip().replace(' ', '').replace('-', '')
        if not code.isdigit():
            await u.message.reply_text("❌ digits only", reply_markup=BACK_KB)
            return
        client = st['client']
        try:
        ...
        try:
            ...
            await c2.connect()
            await c2.sign_in(phone=st['phone'], code=code, phone_code_hash=st['phone_code_hash'])
            me = await c2.get_me()
            fresh = c2.session.save()
            await c2.disconnect()
        except Exception:
            try:
                await client.connect()
        ...
        fname = getattr(me, 'first_name', None) or "User"
        au.append({'id': nid, 'name': fname, 'api_id': st['api_id'], 'api_hash': st['api_hash'],
                   'session_string': fresh, 'phone': st['phone'],
                   'user_id': getattr(me, 'id', None), 'owner_id': st['owner_id'],
                   'login_time': datetime.now().isoformat()})
        save_auth_sessions(au)
        display_names[nid] = fname
        c.user_data['awaiting'] = None
        c.user_data.pop('login_id', None)
        phone_login_states.pop(lid, None)
        refresh_account_stats(st['owner_id'])
    ...
    if aw == '2fa_password':
        ...
        au = load_auth_sessions()
        nid = gen_unique_id("phone", st['owner_id'])
        fname = getattr(me, 'first_name', None) or "Unknown"
        au.append({'id': nid, 'name': fname, 'api_id': st['api_id'], 'api_hash': st['api_hash'],
                   'session_string': fresh, 'phone': st['phone'],
                   'user_id': getattr(me, 'id', None), 'owner_id': st['owner_id'],
                   'login_time': datetime.now().isoformat()})
    ...
    if aw == 'add_account':
        ...
        try:
            ...
            if not ok:
                await sm.edit_text("❌ Invalid/dead session", reply_markup=BACK_KB)
                return
            suc, data = add_dynamic_account(name, fresh, uid)
            await sm.edit_text(f"✅ {name} added!" if suc else f"❌ {data}", reply_markup=BACK_KB)
        except Exception as e:
            await u.message.reply_text(f"❌ {str(e)[:160]}", reply_markup=BACK_KB)
        return
...
async def main():
    ...
    try:
        SHOW_START_TO_OTHERS = json.load(open(data_file)).get('show_start_to_others', True)
    except: pass
    load_data()
    for acc in get_all_accounts():
        aid = acc['id']
        account_stats.setdefault(aid, {'sent': 0, 'running': False, 'failed_channels': []})
        stop_flags[aid] = False
        display_names.setdefault(aid, acc.get('name'))
    valid = {OWNER_ID}
    for a in load_admins():
        if is_valid_admin(a['user_id']): valid.add(a['user_id'])
    for acc in get_all_accounts():
        if acc.get('owner_id', OWNER_ID) not in valid:
    ...
    asyncio.create_task(backup_switch_loop())
    ok = False
    for i in range(5):
        try:
